...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      setBadges(b || []);
      setTotalPoints((b || []).reduce((sum, item) => sum + (item.points || 0), 0));
      setLoading(false);
    };
  ...
  if (loading) return <div className="text-center py-20 font-black text-[#55CDFC] uppercase">Profiel laden...</div>;
  ...
  return (
    <div className="space-y-10 max-w-[800px] mx-auto">
      ...
      <section className="bg-white rounded-3xl p-10 duo-card flex flex-col items-center text-center">
        ...
        <h1 className="text-4xl font-black text-gray-800 tracking-tight">{profile?.display_name || 'Sporter'}</h1>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
          {badges.length > 0 ? badges.map((badge) => (
            <div key={badge.id} className="bg-white rounded-3xl p-8 duo-card flex flex-col items-center text-center group hover:scale-[1.05] transition-all">
              ...
              <p className="text-[10px] font-black text-[#55CDFC] uppercase mt-3 tracking-widest">+{badge.points} XP</p>
            </div>
          )) : (
            <div className="col-span-full bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl p-16 text-center">
              <p className="text-gray-400 font-black uppercase tracking-widest text-xs">Begin een challenge om je<br/>eerste badge te verdienen!</p>
            </div>
          )}
        </div>
      </section>
